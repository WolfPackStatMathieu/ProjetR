...
random_encouragementFR <- function() {
  paste0(sample(random_encouragements, 1))
}
...
Alors comme ça, tu veux apprendre à skimmer ? Bien, tu es au bon endroit. Dans peu de temps, tu pourras toi aussi narguer tes camarades qui utilisent *encore* des commandes de **Rbase**. 
...
L’avantage de `skim()` par rapport à `summary()` est la pertinence des paramètres de dispersion et de position, qui sont les plus usuels pour un statisticien. Chaque type de données est présenté séparément. SkimR peut gérer les valeurs booléennes, les dates et bien d’autres types comme le type numérique, caractère, factor, booléen, complexes, dates, POSIXct[^1], ts[^2] et supporte les spark-bar et les spark-line[^3]. De plus, l’histogramme permet de rapidement regarder si la distribution de la variable choisie s’éloigne d’une distribution normale, souvent plus parlant qu’une p-value.
...
<!-- [^1]:date avec temps, par exemple 2021-02-26 23:45 GMT -->
<!-- [^2]:des séries temporelles -->
<!-- [^3]:graphiques dans les cellules du data frame, qui ne doivent donc pas être considérés comme des valeurs -->

<!--Rappel des différences entre skim et summary-->
Quelles différences observez-vous entre les 2 sorties ? Prenez quelques instants pour réfléchir puis comparez votre réflexion avec les informations suivantes : voici la sortie `summary(hdv2003)` à comparer avec la sortie `skim(hdv2003)` affichée précédemment. 
```{r affichagesummary,  exercise=FALSE,exercise.startover = TRUE,echo=TRUE}
summary(hdv2003)
```
...
on peut remplacer la syntaxe `table_mère$colonne_choisie1` par `table_mère, colonne_choisie1`. 
...
SkimR peut sembler rigide au premier abord. Cependant, il est possible de personnaliser les sorties et, surtout, de préciser les statistiques qui nous intéressent le plus. Nous pouvons donc remplacer, supprimer ou ajouter nos propres statistiques dans le but de simplifier encore plus son utilisation. Pour cela, il faut créer une liste de fonctions skim `sfl()` associée à un typage particulier de données (numeric, factor ou character) puis les implémenter grâce à la fonction `skim_with()`. Pour revenir à tout moment à la configuration de base de `skim_with()`, il est possible de restaurer les paramètres par défaut grâce à la fonction `skim_with_defaults()`. 

<!--encore un rappel de la différence entre summary et skim-->

### Une fonction qui a de l'allure

Désormais, à l’aide des sorties de la fonction `skim()`, que pouvez-vous dire à nouveau sur la distribution de la variable `age` et que ne le permettait pas la fonction `summary()` ?



```{r quizzexo6, echo=FALSE}
quiz(caption = "Une distribution normale ?",
  question(text = "Que nous dit Skim que Summary nous a spolié ?",
    answer(text = "il n’y a aucune différence", message = random_encouragementFR()),
    answer(text = "Oui, et je ne sais pas quoi ajouter", message = random_encouragementFR()),
    answer(text= "Skim fournit un histogramme de la distribution de chaque variable numérique, intégrée à la table des résultats. Cela permet de dire que la distribution de la variable âge est d’allure gaussienne, chose que l’on ne pouvait conclure à travers la fonction summary.", correct = TRUE),
    answer(text = "La seule différence est le format.", message = random_encouragementFR()),
    answer(text="La variable âge est à la fois qualitative et quantitative, ce qui nous pose un problème",message = random_encouragementFR()),
    allow_retry = TRUE,
    random_answer_order = TRUE,
    post_message = random_praiseFR()
...
`skim()` permet de visualiser l’allure de la distribution des variables, et en particulier de la variable `age`. Donc, on peut déduire des histogrammes de sortie que la variable âge est globalement distribuée selon une loi normale.

Voici le code pour vérifier la normalité d’une variable à travers un qq-plot :
```{r exemple_qqplot, exercise=FALSE,exercise.startover = TRUE,echo=TRUE}
# QQ plot
ggqqplot(hdv2003$age)
```

Bien que les queues de distribution, à gauche notamment, s’écartent légèrement de la distribution normale théorique, le qq-plot de la variable âge permet de confirmer la normalité de sa distribution.

<!-- début d’utilisation de skim_with, mise en pratique simpliste du texte précédent l’exo 6 -->
### Déshabiller skim ! 

On souhaite désormais retirer l'histogramme en sortie. Pour cela, il est possible d'utiliser la fonction `skim_without_charts()`. Appliquer cette fonction au jeu de données `hdv2003` 

```{r exo_chart, exercise=TRUE,exercise.startover = TRUE, exercise.eval=FALSE}

```
```{r exo_chart-solution}
skim_without_charts(hdv2003)
```
### Déshabiller skim différemment ! 

Pour enlever l’histogramme, il existe une alternative : 
Utiliser la fonction `skim_with()` qui permet d’implémenter une liste de statistiques à retourner ou non. Ici, dans la liste, on dit que l’on ne veut pas voir le graphe. D’une manière générale, si on souhaite se débarrasser d’un skimmer i.e. une statistique produite par `skim()`, on la fixe `= NULL`. On peut avoir les noms des skimmer de base avec `get_skimmer_names()`
`skim_with()` est une fonction qui produit une fonction, elle est clôturée au sens mathématique ; il est donc possible d’en créer plusieurs dans une même session R. Néanmoins, il faut assigner la sortie de skim_with() avant de ne pouvoir l'utiliser.

Dans l'exercice suivant, en utilisant `skim_with()`, enlever l'histogramme `hist` de la sortie produite

```{r exercice7,exercise=TRUE,exercise.eval=FALSE,exercise.startover = TRUE}
sans_hist<- ...(numeric = sfl(... = ...))
sans_hist(...)
```
...
On choisit pour les variables numériques d’écraser les stats fournies initialement par `skim_with(append=False)` et de mettre l’écart interquartile et l’écart absolu à la médiane. On stocke ce `skim_with()` afin de pouvoir l’utiliser comme une fonction. On  peut utiliser la manière directe, on déclare les nouvelles stats en même temps que le `skim_with()` ou on peut créer la liste de statistiques qu’on stocke, puis on crée le `skim_with()`.

Ici, on ne garde que sd et median, on les liste pour les variables numériques, et on écrase (append=False) 

```{r exemple1, exercise = TRUE, exercise.startover = TRUE, exercise.eval=FALSE,echo=TRUE}
mon_skim <- skim_with(
...
Dans cet exemple, on ne veut qu’une nouvelle variable “écart absolu à la médiane”, en se débarrassant des valeurs manquantes, il faut placer un tild avant la fonction et respecter la syntaxe suivante:  -->

```{r exemple2, exercise=TRUE,exercise.startover = TRUE, exercise.eval=FALSE,echo=TRUE}
...
Une autre façon de coder est de lister les fonctions qu’on va stocker et de les renseigner à l’intérieur du `skim_with()`. Ici, `mes_skimmers` contient les fonctions `mean,max` pour les variables numérique et `is.ordered` pour les factors


```{r exemple3, exercise=TRUE,exercise.startover = TRUE, exercise.eval=FALSE,echo=TRUE}
mes_skimmers<-list(
...
Pour les variables de type numérique, ajoutez l’écart interquartile `IQR` qu'on appelera eiq, en prenant soin de supprimer les valeurs manquantes. Pour les variables de type facteur, déterminez le nombre d’éléments uniques grâce à `n_unique`. La liste de fonction skim sera appelée uniquement dans le `skim_with()`
...
mon_skim_3(hdv2003)
```
```{r exercice8-ecart-hint-2}
mon_skim_3<-skim_with(
  numeric = ...(eiq = ~ IQR(., na.rm = TRUE)),
  ... = sfl(...),append = TRUE)
...
# On utilise la même technique pour ajouter `(append=TRUE)` l’écart interquartile et enlever les NA.
# Pour les factors, n_unique permet de calculer le nombre d’éléments unique et de supprimer les NA.
mon_skim_3<-skim_with(
...
```{r exercice9, exercise=TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
mon_skim_mad <- ...
...
mon_skim_mad <- skim_with(
  ... = ...("écat absolu à la médiane"= ...),
  ... = FALSE)
mon_skim_mad(...)
```
```{r exercice9-hint-2}
mon_skim_mad <- skim_with(
  ... = sfl("écat absolu à la médiane"= mad),
  ... = FALSE)
mon_skim_mad(...)
```
```{r exercice9-solution}
mon_skim_mad <- skim_with(
  numeric = sfl("écat absolu à la médiane"= mad),
  append = FALSE)
mon_skim_mad(hdv2003$poids)
```
## SkimHead ! Le Skim pour les vrais !
...
La syntaxe est la suivante : Chaque ligne faisant parties du bloc à traiter doit impérativement se terminer par le pipeline `%>%`, sauf pour la dernière ligne qui marque la fin du bloc. Cette syntaxe doit être respectée absolument ! 
Les fonctions utilisées doivent être précédées de `dplyr::`
...
Skim fonctionne très bien avec les groupements. Réalisez un `skim()` de la table hdv2003 en regroupant les informations par sexe.  

<div id="exercice-supplementaire">
**WARNING**  n’hésitez pas à réaliser des tutoriels dplyr en amont. La résolution de cet exercice utilise uniquement cette syntaxe, simplifiant énormément le codage.Le paragraphe d'introduction peut néanmoins vous suffire.
</div>

```{r exoh1, exercise=TRUE, exercise.eval=FALSE, exercise.startover = TRUE}
hdv2003 ...
...
```
```{r exoh1-solution}
# on charge hdv2003, on groupe par sexe via dplyr puis on skimme le tout
hdv2003 %>%
  dplyr::group_by(sexe) %>%
  skim()
```
...
Toujours sur la table `hdv2003`, entre la variable comptabilisant le nombre de frères et sœurs et celle comptabilisant le nombre d’heures passées devant la télévision, laquelle a la valeur moyenne la plus élevée (freres.soeurs)?

```{r exoh2, exercise=TRUE, exercise.eval=FALSE, exercise.startover = TRUE}
hdv2003
 
  skim()
```
```{r exoh2-hint-1, exercise=TRUE, exercise.eval=FALSE, exercise.startover = TRUE}
hdv2003...
  ...::...(...,...) ...
  skim()
```

```{r exoh2-hint-2, exercise=TRUE, exercise.eval=FALSE, exercise.startover = TRUE}
hdv2003...
  dplyr::...(freres.soeurs,heures.tv) ...
  skim()
```

```{r exoh2-solution}
# on charge la table, on sélectionne les variables qui nous intéressent puis on skimme et on peut conclure.
hdv2003%>%
...
La table hdv2003 comporte-t-elle des valeurs manquantes (réponse : heures.tv   mauvaises réponses : age, nivetud,hard.rock) ? 
```{r exoh3,exercise=TRUE,exercise.eval=FALSE, exercise.startover = TRUE}

```
```{r exoh3-solution , exercise.reveal_solution =FALSE}
# On charge hdv2003, puis on le skimme et on filtre le skim_df pour n’avoir que les variables ayant des valeurs manquantes
hdv2003 %>%
  skim() %>%
...
```{r quizzexo3, echo=FALSE}
quiz(caption = "Manquantes, vous avez dit manquantes ?",
  question(text = " Y a-t-il des variables de la table hdv2003 ayant des valeurs manquantes ?",
    answer(text = "Non, et je ne sais pas pourquoi", message = "Vérifie tes cours de statistique descriptive !"),
    answer(text = "Oui, et je ne sais pas quoi ajouter", message = "Cherche un peu plus" ),
    answer(text= "Non, on ne peut rien dire sur l’allure générale de la distribution", correct = TRUE),
    answer(text = "La médiane est très proche de la moyenne (signe d’une faible dispersion)", correct = TRUE),
